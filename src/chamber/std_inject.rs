...
pub fn maybe_inject_crates_ref(sess: &Session, krate: ast::Crate, altstd: Option<String>)
                               -> ast::Crate {
    if use_std(&krate) {
        inject_crates_ref(sess, krate, altstd)
    } else {
        krate
    }
}
...
impl<'a> fold::Folder for StandardLibraryInjector<'a> {
    fn fold_crate(&mut self, mut krate: ast::Crate) -> ast::Crate {
        ...
        let actual_crate_name = match self.altstd {
...
fn inject_crates_ref(sess: &Session, krate: ast::Crate,
                     altstd: Option<String>) -> ast::Crate {
